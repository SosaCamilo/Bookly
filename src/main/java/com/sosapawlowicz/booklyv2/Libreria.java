...
import com.sosapawlowicz.booklyv2.Administrador;
import com.sosapawlowicz.booklyv2.Comic;
import com.sosapawlowicz.booklyv2.Libro;
import com.sosapawlowicz.booklyv2.Novela;
import com.sosapawlowicz.booklyv2.Prestamo;
...
public class Libreria {
    ...
    public void addCliente(String nombre, String apellido, String email, int dni, int telefono, String domicilio, Date nacimiento) {
        Cliente clienteNuevo = new Cliente(nombre, apellido, email, dni, telefono, domicilio, nacimiento);
    ...
    public void addNovela(String titulo, String genero, String autor,  String descripcion, int paginas, int capitulos) {
        Novela novelaNueva = new Novela(titulo, genero, autor, descripcion, paginas, capitulos);
        
        libros.add(novelaNueva);

    }
    
    public void addComic(String titulo, String genero, String autor, String descripcion, int paginas, String ilustrador) {
        Comic comicNuevo = new Comic(titulo, genero, autor, descripcion,paginas, ilustrador);
        
        libros.add(comicNuevo);
    ...
    public int addPrestamo(Cliente cliente, Libro libro, Date fechaRetiro, Date fechaVencimiento) {
        int nuevoId = prestamos.size() + 1;
        Prestamo prestamoNuevo = new Prestamo(nuevoId, fechaRetiro, fechaVencimiento, "Activo", cliente, libro);
    ...
    public void mostrarPrestamosVencidos() {
        ...
        boolean hay = false;
        for (Prestamo p : prestamos) {
            if (p.getEstado().equals("Activo") && p.getfechaVencimiento().before(hoy)) {
                hay = true;
                System.out.println("ID: " + p.getIdPrestamo()
                        + " | Cliente: " + p.getCliente().getNombre() + " " + p.getCliente().getApellido()
                        + " | Libro: " + p.getLibro().getTitulo()
                        + " | Vencio: " + p.getfechaVencimiento());
            }
        }
        if (!hay) {
